    ...
    def reachAlloc(addr: Addr): Val = heap(addr) match {
      case VirtualInstance(ArrayKind, cls, values, zoneHandle) =>
        ...
        if (zoneHandle != Val.Null)
          println(
            s"[debug/materialize/reachAlloc] arrayalloc($elemty, $init, ${escapedVal(zoneHandle)})"
          )
      ...
      case VirtualInstance(_, cls, values, zoneHandle) =>
        if (zoneHandle != Val.Null)
          println(
            s"[debug/materialize/reachAlloc] VirtualInstance(_, ${cls.name}, ${values}, ${zoneHandle}))"
          )
